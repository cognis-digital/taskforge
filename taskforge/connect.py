"""Native cognis-connect emit for taskforge — forward findings to any platform.

Maps taskforge's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    taskforge ... --format json | taskforge-emit --to stix
    taskforge-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
